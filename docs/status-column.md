...
This feature adds a small toggle button directly to the management screen. Switch it **on** to publish, or **off** to save as a draft.
...
To add a status column to the table of the post list of a custom post type, when you [create that post type](/custom-post-types/) with Meta Box, navigate to the **Features** tab, then enable the setting as below:

![Enable the status column for the posts of custom post type](./img/status-column/enable-feature.png)

Then, you will see the Status column displayed. Turn the button on to publish or turn it off to save the post to a draft.
